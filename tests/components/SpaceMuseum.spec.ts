...
describe("SpaceMuseum Component Logic", () => {
  ...
  describe("3D Furniture Loading", () => {
    ...
    describe("Model Cleanup", () => {
      ...
      it("should properly dispose of bench model resources", () => {
        const mockGeometry = { dispose: jest.fn() };
        const mockMaterialArray = [{ dispose: jest.fn() }, { dispose: jest.fn() }];
  ...
  describe("Event Handling", () => {
    ...
    it("should expose Back Home button through the existing exit event", () => {
      const buttonConfig = {
        label: "Back Home",
        event: "exit-museum",
        type: "button",
      ...
      expect(buttonConfig.label).toBe("Back Home");
      expect(buttonConfig.event).toBe("exit-museum");
      expect(buttonConfig.type).toBe("button");
    });

    it("should release pointer lock before exiting the museum", () => {
      const mockEmit = jest.fn();
      const mockDocument = {
        exitPointerLock: jest.fn(),
      };
      const state = { isPointerLocked: true };

      const exitMuseum = () => {
        if (state.isPointerLocked) {
          mockDocument.exitPointerLock();
        }
        mockEmit("exit-museum");
      };
  ...
  describe("Jetpack Firing System", () => {
    ...
    describe("Audio System Integration", () => {
      ...
      it("should set correct volume relative to music volume", () => {
        ...
        expect(jetpackSound.volume).toBeCloseTo(0.56); // 0.8 * 0.7
      });
    ...
    describe("Jump Sequence Logic", () => {
      ...
      it("should apply correct jump multiplier for double jump", () => {
        ...
        const firstJumpCount: number = 1;
        const firstJumpMultiplier = firstJumpCount === 2 ? 1.1 : 1.0;
    ...
    describe("Jetpack Sound Stopping", () => {
      ...
      it("should handle null jetpack sound gracefully", () => {
        const stopJetpackSound = (
          jetpackSound: { pause: jest.Mock; currentTime: number } | null,
        ) => {
          if (jetpackSound) {
            jetpackSound.pause();
            jetpackSound.currentTime = 0;
        ...
        expect(() => stopJetpackSound(null)).not.toThrow();
      });
    });
    ...
    describe("Volume Control Integration", () => {
      ...
      it("should work independently of music enabled/disabled state", () => {
        ...
        expect(jetpackSound.volume).toBeCloseTo(0.56); // Works even when music is off
      });
    ...
    describe("Console Logging and Debugging", () => {
      it("should log appropriate messages for jump types", () => {
        const jumpCount1: number = 1;
        const jumpCount2: number = 2;
        ...
        expect(mockConsoleLog).toHaveBeenCalledWith(
          "🚀 Jetpack fired! Double jump activated (jump #2 in sequence)",
        );
      });
      ...
      it("should log space key hold prevention", () => {
        ...
        expect(mockConsoleLog).toHaveBeenCalledWith(
          "🚫 Space held down - release and press again to jump",
        );
      });

...
class Physics { _overlap = jest.fn(); add = { existing: (obj:any)=>{ obj.body = { enable:true, setCircle: jest.fn(), setVelocity: jest.fn(), setAllowRotation: jest.fn() } }, overlap: this._overlap } }
...
export class SceneBase { sys:any; scale:any; add:any; tweens:any; textures:any; physics:any; input:any; time:any; scene:any; game:any; load:any
  constructor(config:any){ this.sys={settings:config}; this.scale={width:1200, height:900}; this.add={ container:(x:number,y:number)=>new Container(x,y), image:()=>new Image(), rectangle:()=>new GameObject(), circle:()=>new GameObject(), polygon:()=>new GameObject(), text:(x:number,y:number,t:string,s?:any)=>new Text(x,y,t,s), graphics:()=>new Graphics(), sprite:()=>new Image(), particles:()=>new GameObject(), group:()=>new Group() }; this.tweens=new Tweens(); this.textures=new Textures(); this.physics=new Physics(); this.input={ keyboard: new Keyboard(), on: jest.fn() }; this.time=new Time(); this.scene={ setActive: jest.fn(), start: jest.fn() }; this.game={ loop:{ delta:16 } }; this.load=new Load(); }
}

const PhaserMock = { Scene: SceneBase as any, GameObjects: { Text }, Math: MathNS, Display, Input, BlendModes, Types: { Physics: { Arcade: {} } } }

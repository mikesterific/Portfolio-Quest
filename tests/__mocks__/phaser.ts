...
export class Vector2 {
  ...
  normalize() {
    const l = Math.sqrt(this.x * this.x + this.y * this.y) || 1;
    this.x /= l;
    this.y /= l;
    return this;
  }
  ...
  lerp(to: Vector2, t: number) {
    this.x = this.x + (to.x - this.x) * t;
    this.y = this.y + (to.y - this.y) * t;
    return this;
  }
}
...
export const MathNS = {
  ...
  Angle: {
    ...
    ShortestBetween: (a: number, b: number) => b - a,
  },
...
export const Display = {
  Color: {
    HexStringToColor: (hex?: string) => ({
      color: parseInt((hex || "#000000").replace("#", ""), 16),
    }),
  },
};
...
export class Text extends GameObject {
  text = "";
  style: any = {};
  constructor(x: number, y: number, t: string, s?: any) {
    super();
    this.x = x;
    this.y = y;
    this.text = t;
...
class Group {
  children = {
    entries: [] as any[],
    each: (cb: Function, ctx: any) => {
      this.children.entries.forEach((e) => cb.call(ctx, e));
    },
  };
  add(o: any) {
    this.children.entries.push(o);
    return this;
  }
  getChildren() {
  ...
  add = {
    existing: (obj: any) => {
      obj.body = {
        ...
        velocity: { x: 0, y: 0 },
        setCircle: jest.fn(),
        setVelocity: jest.fn(function (x?: number, y?: number) {
          if (typeof x === "number") this.velocity.x = x;
          if (typeof y === "number") this.velocity.y = y;
          return this;
        }),
        setVelocityX: jest.fn(function (x: number) {
          this.velocity.x = x;
          return this;
        }),
        setVelocityY: jest.fn(function (y: number) {
          this.velocity.y = y;
          return this;
        }),
        setAllowRotation: jest.fn(),
        setDrag: jest.fn(),
...
class Sound {
  add() {
    return { play: jest.fn(), isPlaying: false, volume: 0.4 };
  }
}
class Load {
  image() {
...
class Events {
  ...
  constructor() {
    this.once.mockReturnThis();
    this.on.mockReturnThis();
    this.emit.mockReturnThis();
    this.off.mockReturnThis();
    this.removeListener.mockReturnThis();
  }
}
...
export class SceneBase {
  ...
  constructor(config: any) {
    ...
    this.scale = { width: 1200, height: 900 };
    this.add = {
      container: (x: number, y: number) => new Container(x, y),
      image: () => new Image(),
      rectangle: () => new GameObject(),
      circle: () => new GameObject(),
      polygon: () => new GameObject(),
      text: (x: number, y: number, t: string, s?: any) => new Text(x, y, t, s),
    ...
    this.input = { keyboard: new Keyboard(), on: jest.fn() };
    this.time = new Time();
    this.scene = {
      setActive: jest.fn(),
      start: jest.fn(),

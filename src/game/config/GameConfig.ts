...
export const validateGameConfig = (config: GameConfigInterface): boolean => {
  ...
  if (player.shields.max <= 0) {
    throw new Error("Player shield max must be greater than 0");
  }
  if (player.shields.regenerationDelayMs < 0) {
    throw new Error("Player shield regeneration delay cannot be negative");
  }
...
export const mergeConfig = (
  ...
  if (overrides.player) {
    ...
    if (overrides.player.health) {
      merged.player.health = { ...merged.player.health, ...overrides.player.health };
    }
    if (overrides.player.shields) {
      merged.player.shields = { ...merged.player.shields, ...overrides.player.shields };
    }
  }
  ...
  if (overrides.combat) {
    merged.combat = {
      ...merged.combat,
      ...overrides.combat,
      ...(overrides.combat.laser && {
        laser: { ...merged.combat.laser, ...overrides.combat.laser },
      }),
      ...(overrides.combat.xp && { xp: { ...merged.combat.xp, ...overrides.combat.xp } }),
      ...(overrides.combat.collision && {
        collision: { ...merged.combat.collision, ...overrides.combat.collision },
      }),
    };
  }

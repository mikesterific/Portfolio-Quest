...
export class GameUIScene extends Phaser.Scene {
  ...
  private setupUI(): void {
    ...
    this.combatButton = this.add
      .text(20, 70, "⚔️ Combat: ON", {
        fontSize: "16px",
        color: "#ffffff",
        padding: { x: 10, y: 5 },
      })
    ...
    this.currentSceneText = this.add
      .text(width / 2, height - 30, "Loading...", {
        fontSize: "18px",
        color: "#ecf0f1",
        padding: { x: 15, y: 8 },
      })
  ...
  private toggleCombatSetting(): void {
    ...
    if (
      !this.combatToggleButton ||
      !this.combatToggleButton.scene ||
      !this.combatToggleButton.active
    ) {
    ...
    gameEventBridge.onGameEvent("game:progress-complete", (data) => {
      try {
        const toast = this.add
          .text(this.scale.width / 2, 80, `All stations unlocked!`, {
            fontSize: "20px",
            color: "#2ecc71",
